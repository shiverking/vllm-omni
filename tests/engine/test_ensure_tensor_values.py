...
"""Unit tests for tensor-only multimodal wire payload enforcement."""
...
def test_scalar_float_wrapped():
    ...
    result = ensure_tensor_values({"gain": 0.5})
...
def test_list_converted():
    ...
    result = ensure_tensor_values({"ids": [1, 2, 3]})
...
def test_non_tensorizable_dropped():
    ...
    result = ensure_tensor_values({"ok": torch.randn(5), "bad": {"nested": "dict"}})
    assert "ok" in result
...
def test_mixed_payload():
    ...
    result = ensure_tensor_values(payload)
    assert len(result) == 4
