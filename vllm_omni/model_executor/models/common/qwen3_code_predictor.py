    ...
    def _capture_npu_graphs(self) -> None:
        ...
        if self._kv_cache_enabled:
            ...
            print(
                ...
                f"graph_keys={sorted(kv_graph_keys)} "
            ...
            if self._fia_gqa_enabled:
                print(
                    ...
                    f"materialized_kv_heads={int(self.config.num_key_value_heads)}",
                    flush=True,
                )
        elif self._prefix_graphs_enabled:
            prefix_seq_lens = self._prefix_seq_lens(max_seq)
    ...
    def forward(
        ...
        for step in range(1, num_groups):
            if use_kv_cache:
                ...
                if cache_len not in self._printed_kv_cache_lens:
                    phase = "prefill" if cache_len == 2 else "decode"
                    input_len = 2 if cache_len == 2 else 1
                    print(
                    ...
                    self._printed_kv_cache_lens.add(cache_len)
                if self._fia_gqa_enabled and cache_len not in self._printed_fia_gqa_cache_lens:
                    phase = "prefill" if cache_len == 2 else "decode"
                    query_len = 2 if cache_len == 2 else 1
                    print(
                        "[Qwen3-TTS][NPU FIA GQA] active "
                        f"batch_bucket={padded_bsz} "
                        f"phase={phase} "
                        f"query_len={query_len} "
                        f"valid_kv_len={cache_len} "
                        f"query_heads={int(self.config.num_attention_heads)} "
                        f"kv_heads={int(self.config.num_key_value_heads)} "
                        "cache_backed=true",
                        flush=True,
                    )
                    self._printed_fia_gqa_cache_lens.add(cache_len)
                ...
                if self._prefix_graphs_enabled:
                    ...
                    if (
                        graph_key == prefix_key
                        and self._is_npu
                        and seq_len not in self._printed_short_prefix_seq_lens
                    ):
                        print(
